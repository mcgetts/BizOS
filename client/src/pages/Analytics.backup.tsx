...
export default function Analytics() {
  ...
  // AI-powered business insights based on real data
  const businessInsights: BusinessInsight[] = React.useMemo(() => {
    const insights: BusinessInsight[] = [];
    const today = new Date();

    // Analyze project performance for insights
    if (projects && projects.length > 0) {
      // Opportunity: High-performing projects
      const completedProjects = projects.filter(p => p.status === 'completed');
      const inProgressProjects = projects.filter(p => p.status === 'in_progress');

      if (completedProjects.length > 0 && clients) {
        // Identify most valuable clients
        const clientProjectCounts = clients.map(client => {
          const clientProjects = completedProjects.filter(p => p.clientId === client.id);
          return { client, projectCount: clientProjects.length };
        }).filter(c => c.projectCount > 0).sort((a, b) => b.projectCount - a.projectCount);

        if (clientProjectCounts.length > 0) {
          const topClient = clientProjectCounts[0];
          insights.push({
            id: "client-opportunity",
            type: "opportunity",
            priority: "high",
            title: "Expand with High-Value Client",
            description: `${topClient.client.name} has completed ${topClient.projectCount} projects successfully`,
            impact: "Potential for additional recurring revenue",
            action: "Schedule strategic account review and propose new services",
            metric: `${topClient.projectCount} completed projects`
          });
        }
      }

      // Risk: Projects approaching deadlines
      const projectsAtRisk = inProgressProjects.filter(p => {
        if (!p.endDate) return false;
        const daysToDeadline = Math.ceil((new Date(p.endDate).getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
        return daysToDeadline <= 14 && daysToDeadline > 0;
      });
      ...
      if (projectsAtRisk.length > 0) {
        insights.push({
          ...
          type: "risk",
          priority: "high",
          title: "Projects Approaching Deadlines",
          description: `${projectsAtRisk.length} projects have deadlines within 2 weeks`,
          impact: "Risk of client dissatisfaction and project delays",
          action: "Review project schedules and reallocate resources if needed",
          metric: `${projectsAtRisk.length} projects at risk`
        });
      }
    }
    ...
    if (tasks && tasks.length > 0) {
      const recentCompletions = tasks.filter(t =>
        ...
        new Date(t.updatedAt) > new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000) // Last 7 days
      );
      ...
      if (recentCompletions.length >= 10) {
        insights.push({
          ...
          description: `Team completed ${recentCompletions.length} tasks in the past week`,
          impact: "Above-average productivity indicating good team performance",
          action: "Consider team recognition and document successful processes",
          metric: `${recentCompletions.length} tasks completed this week`
        });
      }

      // Risk: Blocked tasks accumulating
      if (blockedTasks.length > 0) {
        insights.push({
          id: "blocked-tasks-risk",
          type: "risk",
          priority: "medium",
          title: "Task Bottlenecks Detected",
          description: `${blockedTasks.length} tasks are currently blocked`,
          impact: "Blocking tasks can cascade delays across projects",
          action: "Review and resolve blockers immediately",
          metric: `${blockedTasks.length} blocked tasks`
        });
      }
    }

    // Analyze team composition and workload
    if (users && users.length > 0) {
      const teamMembers = users.filter(u => u.role !== 'admin');
      const managers = users.filter(u => u.role === 'manager');

      // Recommendation: Team structure optimization
      if (teamMembers.length > 8 && managers.length < 2) {
        insights.push({
          id: "team-structure",
          type: "recommendation",
          priority: "medium",
          title: "Consider Team Structure Optimization",
          description: `${teamMembers.length} team members with only ${managers.length} manager(s)`,
          impact: "Better management span could improve efficiency",
          action: "Evaluate promoting senior team members to management roles",
          metric: `${teamMembers.length}:${managers.length} team-to-manager ratio`
        });
      }
    }
    ...
    if (clients && clients.length > 0) {
      ...
      const recentContacts = clients.filter(c =>
        c.lastContactDate && new Date(c.lastContactDate) > new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000)
      );

      // Opportunity: Lead conversion
      if (leads.length > 0 && activeClients.length > 0) {
        const conversionRate = (activeClients.length / (activeClients.length + leads.length)) * 100;
        if (conversionRate < 60) {
          insights.push({
            ...
            description: `Current lead conversion rate is ${conversionRate.toFixed(1)}%`,
            impact: "Improving conversion could significantly increase revenue",
            action: "Review lead nurturing process and follow-up procedures",
            metric: `${conversionRate.toFixed(1)}% conversion rate`
          });
        }
      }

      // Risk: Client communication gaps
      if (recentContacts.length < activeClients.length * 0.7) {
        insights.push({
          id: "client-communication",
          type: "risk",
          priority: "low",
          title: "Client Communication Gap",
          description: `Only ${recentContacts.length} of ${activeClients.length} clients contacted recently`,
          impact: "Infrequent communication may affect client retention",
          action: "Schedule regular client check-ins and implement CRM reminders",
          metric: `${recentContacts.length}/${activeClients.length} clients contacted`
        });
      }
    }
