...
import { exportTest } from "./testing/exportTest.js";
import { userDataExporter } from "./export/userDataExporter.js";
import { createBackupScheduler } from "./backup/backupScheduler.js";
import { workflowAutomation } from "./automation/workflowAutomation.js";
import { businessRulesEngine } from "./automation/businessRulesEngine.js";
import { advancedIntegrationManager } from "./integrations/integrationManager.js";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/metrics', prometheusMetrics.getMetrics.bind(prometheusMetrics));
  app.get('/health/metrics', healthCheckService.metricsCheck.bind(healthCheckService));

  // Monitoring test endpoints (admin only)
  app.get('/admin/monitoring/test', requireRole(['admin', 'super_admin']), async (req, res) => {
    try {
      const results = await monitoringTest.runAllTests();
      const report = monitoringTest.generateReport(results);
  ...
  // Export testing endpoint (admin only)
  app.get('/admin/exports/test', requireRole(['admin', 'super_admin']), async (req, res) => {
    try {
      const results = await exportTest.runAllExportTests();
      const report = exportTest.generateTestReport(results);
      ...
      // Clean up test files after response
      setTimeout(() => {
        exportTest.cleanupTestFiles(results);
      }, 1000);
      ...
      res.status(500).json({
        success: false,
        error: 'Failed to run export tests',
        message: (error as Error).message
      });
    }
  });
  ...
  // User Data Portability Endpoints (GDPR Compliance)
  app.post('/api/user/export-data', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Check if user can include audit logs (admin only)
      if (includeAuditLogs && !['admin', 'super_admin'].includes(req.user.role)) {
        return res.status(403).json({ message: 'Insufficient permissions to export audit logs' });
      }
      ...
      if (result.success) {
        res.json({
          ...
          downloadUrl: `/api/user/download-export/${result.exportId}`,
  ...
  app.get('/api/user/download-export/:exportId', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const filePath = path.join(exportPath, exportFile);
  ...
  app.get('/api/user/export-history', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const history = await userDataExporter.getExportHistory(userId);
      res.json(history);
    } catch (error) {
      sentryService.captureException(error as Error, {
        feature: 'user_export_history',
        userId: req.user?.id
      });
  ...
  app.post('/api/user/request-deletion', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const result = await userDataExporter.requestDataDeletion(userId, reason);

      if (result.success) {
        res.json({
          success: true,
          message: 'Data deletion request submitted successfully. You will be contacted by an administrator to confirm this request.',
      ...
      sentryService.captureException(error as Error, {
        feature: 'user_data_deletion_request',
        userId: req.user?.id
      });
  ...
  // Admin endpoint to cleanup old export files
  app.post('/api/admin/cleanup-exports', requireRole('admin'), async (req, res) => {
    try {
      const { maxAgeHours = 24 } = req.body;
      const cleanedCount = await userDataExporter.cleanupOldExports(maxAgeHours);

      res.json({
        success: true,
        message: `Cleaned up ${cleanedCount} export files`,
        cleanedCount
      });
    } catch (error) {
      sentryService.captureException(error as Error, {
        feature: 'admin_export_cleanup',
        userId: req.user?.id
      });
      res.status(500).json({ message: 'Export cleanup failed' });
    }
  });

  // Workflow Automation & Business Rules API Endpoints
  app.get('/api/automation/workflows', isAuthenticated, async (req, res) => {
    try {
      const triggers = workflowAutomation.getTriggers();
      const metrics = workflowAutomation.getMetrics();
      const businessRulesStats = workflowAutomation.getBusinessRulesStats();
  ...
  app.post('/api/automation/triggers/:triggerId/toggle', requireRole('admin'), async (req, res) => {
    try {
      const { triggerId } = req.params;
      const { isActive } = req.body;

      const success = workflowAutomation.updateTrigger(triggerId, isActive);

      if (success) {
        res.json({
          success: true,
          message: `Trigger ${isActive ? 'enabled' : 'disabled'} successfully`
        });
      } else {
  ...
  app.get('/api/automation/business-rules', isAuthenticated, async (req, res) => {
    try {
      const rules = workflowAutomation.getBusinessRules();
      res.json(rules);
    } catch (error) {
      sentryService.captureException(error as Error, {
      ...
      res.status(500).json({ message: 'Failed to get business rules' });
    }
  });

  app.post('/api/automation/business-rules', requireRole('admin'), async (req, res) => {
    try {
      const rule = req.body;
      ...
      businessRulesEngine.setRule(rule);
      ...
      res.status(500).json({ message: 'Failed to create business rule' });
    }
  });

  app.put('/api/automation/business-rules/:ruleId', requireRole('admin'), async (req, res) => {
    try {
      const { ruleId } = req.params;
      const updatedRule = req.body;

      const existingRule = businessRulesEngine.getRule(ruleId);
      if (!existingRule) {
      ...
      updatedRule.errorCount = existingRule.errorCount;

      businessRulesEngine.setRule(updatedRule);
      ...
      res.status(500).json({ message: 'Failed to update business rule' });
    }
  });

  app.delete('/api/automation/business-rules/:ruleId', requireRole('admin'), async (req, res) => {
    try {
      const { ruleId } = req.params;

      const success = businessRulesEngine.removeRule(ruleId);
      ...
      res.status(500).json({ message: 'Failed to delete business rule' });
    }
  });

  app.post('/api/automation/trigger', isAuthenticated, async (req, res) => {
    try {
      const { eventType, entityData, metadata } = req.body;

      if (!eventType || !entityData) {
        return res.status(400).json({ message: 'Event type and entity data are required' });
      }

      await workflowAutomation.triggerWorkflow(
        eventType,
        entityData,
        req.user.id,
        metadata
  ...
  app.get('/api/automation/metrics', isAuthenticated, async (req, res) => {
    try {
      const metrics = workflowAutomation.getMetrics();
      const businessRulesStats = workflowAutomation.getBusinessRulesStats();
  ...
  // Integration with existing entity operations to trigger workflows
  app.post('/api/projects/:id/trigger-workflow', isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;
      ...
      await workflowAutomation.triggerWorkflow(
        eventType,
        { ...project[0], projectId: id },
        req.user.id
      );

      res.json({
        success: true,
        message: 'Project workflow triggered successfully'
      });
    } catch (error) {
      sentryService.captureException(error as Error, {
        feature: 'project_workflow',
        userId: req.user?.id
      });
      res.status(500).json({ message: 'Failed to trigger project workflow' });
    }
  });

  app.post('/api/tasks/:id/trigger-workflow', isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;
      const { eventType } = req.body;
      ...
      res.json({
        success: true,
        message: 'Task workflow triggered successfully'
      });
    } catch (error) {
      sentryService.captureException(error as Error, {
        feature: 'task_workflow',
        userId: req.user?.id
      });
      res.status(500).json({ message: 'Failed to trigger task workflow' });
    }
  });

  // Advanced Integration Management API Endpoints
  app.get('/api/integrations', isAuthenticated, async (req, res) => {
    try {
      const configurations = advancedIntegrationManager.getConfigurations();
      const systemStatus = advancedIntegrationManager.getSystemStatus();
      ...
      sentryService.captureException(error as Error, {
        feature: 'integration_management',
        userId: req.user?.id
      });
  ...
  app.get('/api/integrations/:type', isAuthenticated, async (req, res) => {
    try {
      ...
      const configuration = advancedIntegrationManager.getConfiguration(type as any);

      if (!configuration) {
        return res.status(404).json({ message: 'Integration not found' });
      }

      const metrics = advancedIntegrationManager.getMetrics(type as any);
      const events = advancedIntegrationManager.getEvents(type as any, 50);
      ...
      sentryService.captureException(error as Error, {
        feature: 'integration_details',
        userId: req.user?.id
      });
      res.status(500).json({ message: 'Failed to get integration details' });
    }
  });

  app.post('/api/integrations/:type/toggle', requireRole('admin'), async (req, res) => {
    try {
      const { type } = req.params;
      const { enabled } = req.body;

      const success = await advancedIntegrationManager.toggleIntegration(
        type as any,
        enabled,
        req.user.id
      );
      ...
      if (success) {
        ...
        res.status(400).json({ message: 'Failed to toggle integration' });
      }
    } catch (error) {
      sentryService.captureException(error as Error, {
        feature: 'integration_toggle',
        userId: req.user?.id
      });
  ...
  app.put('/api/integrations/:type/config', requireRole('admin'), async (req, res) => {
    try {
      const { type } = req.params;
      const updates = req.body;

      const success = await advancedIntegrationManager.updateConfiguration(
        type as any,
        updates,
        req.user.id
      );
      ...
      if (success) {
        ...
        res.status(400).json({ message: 'Failed to update integration configuration' });
      }
    } catch (error) {
      sentryService.captureException(error as Error, {
        feature: 'integration_config_update',
        userId: req.user?.id
      });
  ...
  app.post('/api/integrations/:type/test', requireRole('admin'), async (req, res) => {
    try {
      const { type } = req.params;

      // Perform health check for the integration
      await advancedIntegrationManager.performHealthCheck(type as any);

      const configuration = advancedIntegrationManager.getConfiguration(type as any);

      if (configuration && configuration.healthStatus === 'healthy') {
        res.json({
          success: true,
          message: 'Integration test successful',
          status: configuration.healthStatus
        });
      } else {
        res.status(400).json({
          success: false,
  ...
  app.post('/api/integrations/:type/send-message', isAuthenticated, async (req, res) => {
    try {
      const { type } = req.params;
      ...
      const success = await advancedIntegrationManager.sendMessage(
        type as any,
        channel,
        message,
        options
      ...
      sentryService.captureException(error as Error, {
        feature: 'integration_send_message',
        userId: req.user?.id
      });
  ...
  app.get('/api/integrations/:type/metrics', isAuthenticated, async (req, res) => {
    try {
      const { type } = req.params;
      const metrics = advancedIntegrationManager.getMetrics(type as any);
      ...
      sentryService.captureException(error as Error, {
        feature: 'integration_metrics',
        userId: req.user?.id
      });
      res.status(500).json({ message: 'Failed to get integration metrics' });
    }
  });

  app.get('/api/integrations/:type/events', isAuthenticated, async (req, res) => {
    try {
      const { type } = req.params;
      const { limit = 100 } = req.query;

      const events = advancedIntegrationManager.getEvents(type as any, Number(limit));

      res.json(events);
    } catch (error) {
      sentryService.captureException(error as Error, {
        feature: 'integration_events',
        userId: req.user?.id
      });
      res.status(500).json({ message: 'Failed to get integration events' });
    }
  });

  app.post('/api/integrations/webhook/:type', async (req, res) => {
    try {
      const { type } = req.params;

      const payload = {
        source: type as any,
        eventType: req.headers['x-event-type'] || 'unknown',
        timestamp: new Date(),
        data: req.body,
        signature: req.headers['x-signature'] as string,
        deliveryId: req.headers['x-delivery-id'] as string
      };

      const processed = await advancedIntegrationManager.handleWebhook(payload);
      ...
      sentryService.captureException(error as Error, {
        feature: 'integration_webhook',
        additionalData: { type: req.params.type }
      });
      res.status(500).json({ message: 'Webhook processing failed' });
    }
  });

  app.get('/api/integrations/system/status', isAuthenticated, async (req, res) => {
    try {
      const systemStatus = advancedIntegrationManager.getSystemStatus();
      const allMetrics = advancedIntegrationManager.getMetrics();
      ...
      sentryService.captureException(error as Error, {
        feature: 'integration_system_status',
        userId: req.user?.id
      });
  ...
  app.post('/api/exports', isAuthenticated, async (req, res) => {
    try {
      const { format = 'json', entities = ['all'], dateRange, compressed = false, includeDeleted = false } = req.body;

      // Import the data exporter
      const { exportData } = await import('./export/dataExporter.js');
  ...
  // User Data Portability API endpoints (GDPR compliance)
  app.get('/api/user/export-history', isAuthenticated, async (req, res) => {
    try {
      // For now, return mock data - in production, this would query an exports log table
      const mockHistory = [
        {
          exportId: `export-${Date.now()-86400000}`,
          format: 'json',
  ...
  app.post('/api/user/export-data', isAuthenticated, async (req, res) => {
    try {
      const { format = 'json', includePersonalData = true, includeActivityData = true, includeAuditLogs = false, dateRange, compressed = false } = req.body;

      // Import the user data exporter
      const { exportUserData } = await import('./export/userDataExporter.js');
  ...
  app.get('/api/user/download-export/:exportId', isAuthenticated, async (req, res) => {
    try {
      const { exportId } = req.params;
      const filePath = `/tmp/user-exports/${req.user.id}/${exportId}.json`;

      res.download(filePath, `user-data-${exportId}.json`, (err) => {
        if (err) {
          console.error('Download error:', err);
          res.status(404).json({ message: 'File not found' });
        }
  ...
  app.post('/api/user/request-deletion', isAuthenticated, async (req, res) => {
    try {
      // In a production system, this would:
      // 1. Log the deletion request
      // 2. Send confirmation email
      // 3. Schedule deletion after legal retention period
      // 4. Notify administrators

      console.log(`User ${req.user.id} requested account deletion`);

      // For now, return success - actual deletion would be handled by admin processes
      res.json({
        success: true,
  ...
  app.get('/api/automation/workflows', isAuthenticated, async (req, res) => {
    try {
      // Import workflow automation service
      const { workflowAutomation } = await import('./automation/workflowAutomation.js');

      const workflows = await workflowAutomation.getWorkflows();
      res.json(workflows);
    } catch (error) {
  ...
  app.get('/api/automation/business-rules', isAuthenticated, async (req, res) => {
    try {
      // Import business rules engine
      const { businessRulesEngine } = await import('./automation/businessRulesEngine.js');

      const rules = await businessRulesEngine.getActiveRules();
      res.json(rules);
    } catch (error) {
  ...
  app.post('/api/automation/triggers/:triggerId/toggle', isAuthenticated, async (req, res) => {
    try {
      const { triggerId } = req.params;
      const { enabled } = req.body;
      ...
      const result = await workflowAutomation.toggleTrigger(triggerId, enabled);
      res.json(result);
    } catch (error) {
  ...
  // AI Business Engine insights API endpoints
  app.get('/api/insights', isAuthenticated, async (req, res) => {
    try {
      const { businessInsightsEngine } = await import('./ai/businessInsightsEngine.js');

      const { category, type, limit = 50 } = req.query;
      const insights = businessInsightsEngine.getInsights(
        category as string,
        type as string,
  ...
  app.get('/api/insights/performance-metrics', isAuthenticated, async (req, res) => {
    try {
      const { businessInsightsEngine } = await import('./ai/businessInsightsEngine.js');

      const metrics = await businessInsightsEngine.getPerformanceMetrics();
      res.json(metrics);
    } catch (error) {
  ...
  app.get('/api/insights/summary', isAuthenticated, async (req, res) => {
    try {
      const { businessInsightsEngine } = await import('./ai/businessInsightsEngine.js');

      const summary = businessInsightsEngine.getAnalysisSummary();
      res.json(summary);
    } catch (error) {
      console.error('Failed to get insights summary:', error);
      res.status(500).json({ message: 'Failed to get insights summary' });
    }
  });

  app.post('/api/insights/analyze', isAuthenticated, requireRole(['admin', 'manager']), async (req, res) => {
    try {
      const { businessInsightsEngine } = await import('./ai/businessInsightsEngine.js');

      // Trigger a full analysis
      await businessInsightsEngine.runFullAnalysis();

      res.json({ success: true, message: 'Business analysis initiated' });
    } catch (error) {
      console.error('Failed to run business analysis:', error);
      res.status(500).json({ message: 'Failed to run business analysis' });
    }
  });

  // QuickBooks Online API endpoints
  app.get('/api/quickbooks/auth-url', isAuthenticated, requireRole(['admin', 'manager']), async (req, res) => {
    try {
      const { createQuickBooksIntegration } = await import('./integrations/quickbooksIntegration.js');
      const qb = createQuickBooksIntegration();

      const authUrl = qb.generateAuthUrl();
      res.json({ authUrl });
    } catch (error) {
  ...
  app.post('/api/quickbooks/callback', isAuthenticated, requireRole(['admin', 'manager']), async (req, res) => {
    try {
      ...
      const result = await qb.exchangeCodeForToken(code, state);

      if (result.success) {
        res.json({ success: true, message: 'QuickBooks connected successfully' });
      } else {
  ...
  app.get('/api/quickbooks/status', isAuthenticated, async (req, res) => {
    try {
      ...
      const status = qb.getConnectionStatus();
      res.json(status);
    } catch (error) {
  ...
  app.post('/api/quickbooks/test', isAuthenticated, requireRole(['admin', 'manager']), async (req, res) => {
    try {
      const { createQuickBooksIntegration } = await import('./integrations/quickbooksIntegration.js');
      const qb = createQuickBooksIntegration();
  ...
  app.post('/api/quickbooks/sync/customers', isAuthenticated, requireRole(['admin', 'manager']), async (req, res) => {
    try {
      const { createQuickBooksIntegration } = await import('./integrations/quickbooksIntegration.js');
      const qb = createQuickBooksIntegration();
      ...
      res.json({
        success: true,
        message: `Synchronized ${result.synced} customers with ${result.errors} errors`,
        ...result
      });
    } catch (error) {
      console.error('QuickBooks customer sync failed:', error);
      res.status(500).json({ success: false, message: 'Customer sync failed' });
    }
  });

  app.post('/api/quickbooks/sync/invoices', isAuthenticated, requireRole(['admin', 'manager']), async (req, res) => {
    try {
      const { createQuickBooksIntegration } = await import('./integrations/quickbooksIntegration.js');
      const qb = createQuickBooksIntegration();

      const result = await qb.syncInvoices();
      res.json({
        success: true,
        message: `Synchronized ${result.synced} invoices with ${result.errors} errors`,
        ...result
      });
    } catch (error) {
      console.error('QuickBooks invoice sync failed:', error);
      res.status(500).json({ success: false, message: 'Invoice sync failed' });
    }
  });

  // Stripe Payment Processing API endpoints
  app.get('/api/stripe/config', isAuthenticated, async (req, res) => {
    try {
      const { createStripeIntegration } = await import('./integrations/stripeIntegration.js');
      const stripe = createStripeIntegration();

      const config = stripe.getPublicConfig();
      res.json(config);
    } catch (error) {
  ...
  app.post('/api/stripe/payment-intent', isAuthenticated, async (req, res) => {
    try {
      ...
      const result = await stripe.createPaymentIntent(invoiceId, returnUrl);
  ...
  app.post('/api/stripe/checkout-session', isAuthenticated, async (req, res) => {
    try {
      ...
      const result = await stripe.createCheckoutSession(invoiceId, successUrl, cancelUrl);
      ...
      res.status(500).json({ success: false, message: 'Checkout session creation failed' });
    }
  });
  ...
  app.get('/api/stripe/payment-history/:invoiceId', isAuthenticated, async (req, res) => {
    try {
      ...
      const result = await stripe.getPaymentHistory(invoiceId);
      res.json(result);
    } catch (error) {
  ...
  app.post('/api/stripe/refund', isAuthenticated, requireRole(['admin', 'manager']), async (req, res) => {
    try {
      ...
      const result = await stripe.processRefund(paymentIntentId, amount);
      res.json(result);
    } catch (error) {
  ...
  app.post('/api/stripe/test', isAuthenticated, requireRole(['admin', 'manager']), async (req, res) => {
    try {
      const { createStripeIntegration } = await import('./integrations/stripeIntegration.js');
      const stripe = createStripeIntegration();
